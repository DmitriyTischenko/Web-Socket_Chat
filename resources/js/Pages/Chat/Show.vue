    ...
    created() {
        window.Echo.channel(`store-message.${this.chat.id}`)
            .listen('.store-message', res => {
                if (res.message.user_id !== this.$page.props.auth.user.id) {
                    res.message.is_owner = false; // Не текущий пользователь
            ...
            <div class="mb-4" v-if="messages">
                <div v-if="!isLastPage" class="text-center mb-2">
                    <a @click.prevent="getMessages"
                       class="inline-block bg-sky-600 text-white test-xs px-3 py-2 rounded-lg" href="#">Load more</a>
                </div>
                ...
                <div>
                    ...
                    <div>
                        <a @click.prevent="store"
                           class="inline-block bg-indigo-600 text-white test-xs px-3 py-2 rounded-lg" href="#">Send</a>
                    </div>
                </div>
            </div>
        </div>
